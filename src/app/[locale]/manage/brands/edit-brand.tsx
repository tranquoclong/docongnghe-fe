...
import { UpdateBrandBody, UpdateBrandBodyType } from '@/schemaValidations/brand.schema'
import { useGetBrandQuery, useUpdateBrandMutation } from '@/queries/useBrand'
...
export default function EditBrand({
  ...
  const { data } = useGetBrandQuery({ enabled: Boolean(id), id: id as number })
  const form = useForm<UpdateBrandBodyType>({
    resolver: zodResolver(UpdateBrandBody) as any,
    defaultValues: {
      name: '',
      logo: ''
    }
  ...
  const onSubmit = async (values: UpdateBrandBodyType) => {
    ...
    try {
      ...
      await updateBrandMutation.mutateAsync(body)
  ...
  return (
    ...
    >
      <DialogContent className='sm:max-w-[600px] max-h-screen overflow-auto'>
        ...
        <DialogFooter>
          <Button type='submit' form='edit-Brand-form' disabled={updateBrandMutation.isPending}>
            {updateBrandMutation.isPending && <LoaderCircle className='w-5 h-5 mr-2 animate-spin' />} Lưu
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
